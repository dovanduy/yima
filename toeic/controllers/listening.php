...
class Listening extends CI_Controller {
    ...
    private function get_listening($lid) {
        $query = $this->db->query('SELECT *, (select v.limg from ' . $this->db->dbprefix('kto_toefl_listening') . ' v where v.lid=l.id order by time limit 0,1) as video FROM ' . $this->db->dbprefix('listening') . ' l where l.id=' . $lid);
        foreach ($query->result() as $row) {
            ...
            if (file_exists($f)) {
                ...
                if ($a['Encoding'] == 'Unknown') {
                    ...
                    $response['lsound_duration'] = intval($a['Length']) + 3;
                }
            } else {
                $response['lsound_duration'] = 3;
            }
    ...
    private function get_video($lid) {
        $question;
        $query = $this->db->query('SELECT * FROM ' . $this->db->dbprefix('kto_toefl_listening') . ' where deleted=0 and lid=' . $lid . ' order by time');
    ...
    private function get_scq($lid) {
        $question;
        $query = $this->db->query('SELECT * FROM ' . $this->db->dbprefix('kto_toefl_listening_scq') . ' where deleted=0 and lid=' . $lid);
        foreach ($query->result_array() as $rows) {
            $f = 'admin/data/sounds/listening/scq/' . $rows['lsound'];
            if (file_exists($f)) {
                $this->mp3file->read_newfile($f);
                $a = $this->mp3file->get_metadata();
                $length = intval($a['Length']) + 3;
            } else {
                $length = 3;
            }
            ...
            if (file_exists($f)) {
                $this->mp3file->read_newfile($f);
                $a = $this->mp3file->get_metadata();
                $sentence_length = intval($a['Length']) + 3;
            } else {
                $sentence_length = 3;
            }
    ...
    private function get_mcq($lid) {
        $question;
        $query = $this->db->query('SELECT * FROM ' . $this->db->dbprefix('kto_toefl_listening_mcq') . ' where deleted=0 and lid=' . $lid);
        foreach ($query->result_array() as $rows) {
    ...
    private function get_cq($lid) {
        $question;
        $query = $this->db->query('SELECT * FROM ' . $this->db->dbprefix('kto_toefl_listening_cq') . ' where deleted=0 and lid=' . $lid);
        foreach ($query->result_array() as $rows) {
            ...
            if (isset($col))
                unset($col);
            $query1 = $this->db->query('SELECT * FROM ' . $this->db->dbprefix('kto_toefl_listening_cq_column') . ' where deleted=0 and cqid=' . $rows['id']);
            ...
            $query1 = $this->db->query('SELECT * FROM ' . $this->db->dbprefix('kto_toefl_listening_cq_row') . ' where deleted=0 and cqid=' . $rows['id']);
            foreach ($query1->result_array() as $rows1) {
