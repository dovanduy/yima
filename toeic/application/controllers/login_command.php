...
class Login_command extends CI_Controller {
    ...
    public function index() {
        ...
        switch ($action) {
            ...
            case 'select_session':
                $sql = 'SELECT *, (select t.title from ' . $this->db->dbprefix('test_order') . ' t 
                    where t.id=ss.test_order limit 0,1) as test_order_value
                ...
                $sql1 = 'SELECT * FROM ' . $this->db->dbprefix('mixed_test') . ' mt where mt.id=(select mixed_test from 
                    ' . $this->db->dbprefix('session_student') . ' ss where session="' . $session_id . '" and student="' . $_SESSION['student_id'] . '"  limit 0,1)';
            ...
            case 'check_login':
                $sql = 'SELECT * FROM ' . $this->db->dbprefix('user') . ' where title="' . $username . '" and password="' . md5($password) . '"';
