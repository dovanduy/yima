...
class Listening extends CI_Controller {
    ...
    private function get_listening($lid) {
        $query = $this->db->query('SELECT *, (select v.limg from ' . $this->db->dbprefix('yima_toeic_listening_video') . ' v where v.lid=l.id order by time limit 0,1) as video FROM ' . $this->db->dbprefix('listening') . ' l where l.id=' . $lid);
    ...
    private function get_scq($lid) {
        $question;
        $query = $this->db->query('SELECT * FROM ' . $this->db->dbprefix('yima_toeic_listening_scq') . ' where deleted=0 and lid=' . $lid);
        foreach ($query->result_array() as $rows) {
