...
class UserModel extends CFormModel {
    ...
    public function get($id) {
        $sql = "SELECT ysu.*,yt.total as amount,buy.total as paid
                FROM yima_sys_user ysu
                LEFT JOIN (SELECT SUM(amount) as total,user_id
                            FROM yima_transactions
                            GROUP BY user_id) yt
                ON yt.user_id = ysu.id
                LEFT JOIN (SELECT SUM(amount_vnd) as total,user_id
                            FROM yima_trackings  
                            WHERE completed = 1
                            AND deleted = 0
                            GROUP BY user_id) buy
                ON buy.user_id = ysu.id
                WHERE id = :id
                AND disabled = 0
                AND deleted = 0
                ";
