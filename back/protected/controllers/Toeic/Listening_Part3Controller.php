...
class Listening_Part3Controller extends Controller {

    private $viewData;
    private $message = array('success' => true, 'error' => array());
    private $validator;
    private $Listening_Part3Model;
    ...
    public function init() {
        ...
        /* @var $this ListeningController */
        $this->Listening_Part3Model = new Toeic_Listening_Part3_Model();
    ...
    public function actionIndex($lid, $p = 1) {
        ...
        $listening_part3 = $this->Listening_Part3Model->gets($args, $p, $ppp);
        $total_listening_part3 = $this->Listening_Part3Model->counts($args);
        $listening = $this->ListeningModel->get($listening_id);

        $this->viewData['listening_part3'] = $listening_part3;
        $this->viewData['total'] = $total_listening_part3;
        $this->viewData['listening'] = $listening;
        $this->viewData['paging'] = $total_listening_part3 > $ppp ? HelperApp::get_paging($ppp, Yii::app()->request->baseUrl . "/toeic/listening_part3/index/lid/" . $listening_id . "/p/", $total_listening_part3, $p) : "";
        $this->render('index', $this->viewData);
    }

    public function actionEdit($lid, $id = 0) {
        $this->CheckPermission();
        $listening_part3 = $this->Listening_Part3Model->get($id);
        if (!$listening_part3) {
            $this->layout = "404";
            return;
        }
        ...
        $listening = $this->ListeningModel->get($listening_id);
        if ($_POST)
            $this->do_edit($listening_id, $listening_part3);
    ...
    private function do_edit($listening_id, $listening_part3) {
        $lsound = $listening_part3['lsound'];
        ...
        $this->Listening_Part3Model->update(array('lsound' => $lsound, 'title' => $sound_title, 'question_1' => $question_1, 'choice1_1' => $choice1_1, 'choice2_1' => $choice2_1, 'choice3_1' => $choice3_1, 'choice4_1' => $choice4_1, 'answer_1' => $answer_1,
            'question_2' => $question_2, 'choice1_2' => $choice1_2, 'choice2_2' => $choice2_2, 'choice3_2' => $choice3_2, 'choice4_2' => $choice4_2, 'answer_2' => $answer_2,
            'question_3' => $question_3, 'choice1_3' => $choice1_3, 'choice2_3' => $choice2_3, 'choice3_3' => $choice3_3, 'choice4_3' => $choice4_3, 'answer_3' => $answer_3, 'disabled' => $_POST['disabled'], 'deleted' => $_POST['deleted'], 'last_modified' => time(), 'id' => $listening_part3['id']));
        HelperGlobal::add_log(UserControl::getId(), $this->controllerID(), $this->methodID(), array('Hành động' => 'Sửa', 'Dữ liệu cũ' => $listening_part3, 'Dữ liệu mới' => $_POST));
        $this->redirect(Yii::app()->request->baseUrl . "/toeic/listening_part3/edit/lid/" . $listening_id . "/id/$listening_part3[id]/?s=1");
    }
    ...
    public function actionDelete($lid, $id) {
        ...
        if (!$listening_part3)
            return;
        $listening_id = $lid;
        $this->Listening_Part3Model->update(array('deleted' => 1, 'id' => $id));
    ...
    private function do_add($listening_id) {
        ...
        $lsound = HelperApp::upload_audio($file);
        $listening_part3_id = $this->Listening_Part3Model->add($listening_id, $sound_title, $lsound, $question_1, $choice1_1, $choice2_1, $choice3_1, $choice4_1, $answer_1, $question_2, $choice1_2, $choice2_2, $choice3_2, $choice4_2, $answer_2, $question_3, $choice1_3, $choice2_3, $choice3_3, $choice4_3, $answer_3, time());
        HelperGlobal::add_log(UserControl::getId(), $this->controllerID(), $this->methodID(), array('Hành động' => 'Thêm', 'Dữ liệu' => $_POST));
        $this->redirect(Yii::app()->request->baseUrl . "/toeic/listening_part3/edit/lid/" . $listening_id . "/id/$listening_part3_id/?s=1");
    }
