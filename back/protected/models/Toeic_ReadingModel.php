...
class Toeic_ReadingModel {
    ...
    public function gets($args, $page = 1, $ppp = 20) {
        ...
        if (isset($args['s']) && $args['s'] != "") {
            $custom = " AND ktr.title like :title";
            $params[] = array('name' => ':title', 'value' => "%$args[s]%", 'type' => PDO::PARAM_STR);
        }
        $sql = "SELECT *
                FROM yima_toeic_reading ktr 
                WHERE 1
                $custom
                ORDER BY ktr.title ASC
                LIMIT :page,:ppp";
    ...
    public function counts($args) {
        ...
        $sql = "SELECT count(ktr.id) as total
                FROM yima_toeic_reading ktr 
                WHERE 1
                $custom
                ORDER BY ktr.title ASC";
    ...
    public function add($title, $slug, $keyword, $source, $date_added) {
        $sql = "INSERT into yima_toeic_reading(title, slug, keyword, source, date_added) values (:title, :slug, :source, :keyword, :date_added)";
        $command = Yii::app()->db->createCommand($sql);
    ...
    public function update($args) {
        ...
        $sql = 'update yima_toeic_reading set ' . $custom . ' where id = :id';
        $command = Yii::app()->db->createCommand($sql);
