...
class UserModel extends CFormModel {
    ...
    public function add($title, $password, $secret_key, $role, $firstname, $last_name, $email, $img, $thumbnail, $time) {
        $sql = "INSERT INTO yima_sys_user(title, password, secret_key, role, firstname, lastname, email, date_added, img, thumbnail) VALUES(:title, :password, :secret_key, :role, :firstname, :lastname, :email, :date_added, :img, :thumbnail)";
        $command = Yii::app()->db->createCommand($sql);
