...
<hr/>
<p class="clearfix"><div class="btn btn-primary add-new-score pull-right">Add New Score</div></p>
    ...
    <tbody>
        ...
        for ($i = 1; $i < 5; $i++) {
            ...
            <tr>
                ...
                <td><a class="btn btn-small configure-score" href="<?php echo Yii::app()->request->baseUrl . "/toefl/listening_cq/edit_configure_score/score_id/" . $score_id[$i]; ?>">Edit</a></td>
                <?php if (isset($score_id[$i]) && isset($choices[$i]) && $choices[$i] > 0): ?>
                    <td><a class="btn btn-small btn-danger delete-row" href="<?php echo Yii::app()->request->baseUrl . "/toefl/listening_cq/delete_score/score_id/" . $score_id[$i]; ?>">Delete</a></td>
                <?php else: ?>
