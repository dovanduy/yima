...
class Listening_scq_model extends CI_Model {
    ...
    function count_all_entries() {
        ...
        $this->db->order_by("id", "asc");
        $this->db->order_by("title", "asc");
        return $this->db->count_all_results('listening_scq');
    }
    ...
    function get_entries($start = 0) {
        ...
        $this->db->order_by("id", "asc");
        $this->db->order_by("title", "asc");
        $query = $this->db->get('listening_scq', $per_page, $start);
        return $query->result();
    }
    ...
    function insert_entry() {
        ...
        $this->db->insert('listening_scq', $data);
        ...
        if (isset($_FILES["lsound"]) && $_FILES["lsound"]["tmp_name"] != "") {
            ...
            $filename = "data/sounds/listening/scq/" . $newname;
            ...
            $this->db->update('listening_scq', $data1, array('id' => $id));
        }
		...
		if (isset($_FILES["replay_sound"]) && $_FILES["replay_sound"]["tmp_name"] != "") {
            ...
            $filename = "data/sounds/listening/replay_sound/scq/" . $newname;
            if (file_exists($filename)) {
                unlink($filename);
            }
		...
		if (isset($_FILES["sentence_sound"]) && $_FILES["sentence_sound"]["tmp_name"] != "") {
            ...
            $filename = "data/sounds/listening/sentence_sound/scq/" . $newname;
            if (file_exists($filename)) {
                unlink($filename);
            }
    ...
    function update_entry() {
        ...
        $this->db->update('listening_scq', $data, array('id' => $this->input->post('id')));
    ...
    function delete_entry($id) {
        ...
        $this->db->where('id', $id);
        $this->db->update('listening_scq', $data);
    }
