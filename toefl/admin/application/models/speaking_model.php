...
class Speaking_model extends CI_Model {
    ...
    function get_all_entries() {
        ...
        $this->db->where('speaking_part', $this->session->userdata('part'));
    ...
    function count_all_entries() {
        ...
        $this->db->order_by("disabled", "asc");
        $this->db->order_by("title", "asc");
        return $this->db->count_all_results('speaking');
    }
    ...
    function get_entries($start = 0) {
        ...
        $this->db->order_by("disabled", "asc");
        $this->db->order_by("title", "asc");
        $query = $this->db->get('speaking', $per_page, $start);
        return $query->result();
    }
    ...
    function check_upload_file($id) {
        if (isset($_FILES["limg"]) && $_FILES["limg"]["tmp_name"] != "") {
            ...
            $filename = "data/images/speaking/" . $newname;
            ...
            $data1->limg = $newname;
            $this->db->update('speaking', $data1, array('id' => $id));
        }

        if (isset($_FILES["lsound"]) && $_FILES["lsound"]["tmp_name"] != "") {
            $temp = strrchr($_FILES["lsound"]["name"], ".");
            $tail = substr($temp, 1, strlen($temp) - 1);
            $newname = 'speaking_lsound' . $id . '.' . $tail;
            $filename = "data/sounds/speaking/" . $newname;
            if (file_exists($filename)) {
                unlink($filename);
            }
        ...
        if (isset($_FILES["ssound"]) && $_FILES["ssound"]["tmp_name"] != "") {
            ...
            $tail = substr($temp, 1, strlen($temp) - 1);
            $newname = 'speaking_ssound' . $id . '.' . $tail;
            $filename = "data/sounds/speaking/" . $newname;
            if (file_exists($filename)) {
                unlink($filename);
            }
        ...
        if (isset($_FILES["dsound"]) && $_FILES["dsound"]["tmp_name"] != "") {
            ...
            $tail = substr($temp, 1, strlen($temp) - 1);
            $newname = 'speaking_dsound' . $id . '.' . $tail;
            $filename = "data/sounds/speaking/" . $newname;
            if (file_exists($filename)) {
                unlink($filename);
            }
    ...
    function insert_entry() {
        ...
        $data->speaking_part = $this->session->userdata('part');
    ...
    function update_entry() {
        ...
        $this->db->update('speaking', $data, array('id' => $this->input->post('id')));
