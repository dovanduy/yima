                        ...
                        if (isset($classes)) {
                            foreach ($classes as $item) {
                                ...
                                <tr id="item<?php echo $item['id']; ?>">
                                    <td class="center <?php if (isset($session_class[$item['id']]) && $session_class[$item['id']] == 1) echo 'featured'; ?>"><?php echo $item['title']; ?></td>
                                    <td class="center <?php if (isset($session_class[$item['id']]) && $session_class[$item['id']] == 1) echo 'featured'; ?>">
                                        <?php
                                        if (isset($session_class[$item['id']]) && $session_class[$item['id']] == 1)
                                            echo 'In Session';
                                        ?>
                                    </td>
                                    <td class="center">
...
<script>
    $().ready(function(){
        $('.remove_from_session').live('click',function(){
            var url = '<?php echo base_url(); ?>session/class_session/remove';
            var session_id = <?php echo $session_id; ?>;
            var class_id = $(this).attr('alt');
            $.post(url,{session_id:session_id, class_id:class_id}, function(response) {
                $(location).attr('href','<?php echo $link_current; ?>');
            }, 'json');
        });
        
        $('.add_to_session').live('click',function(){
            var url = '<?php echo base_url(); ?>session/class_session/add';
            var session_id = <?php echo $session_id; ?>;
            var class_id = $(this).attr('alt');
            $.post(url,{session_id:session_id, class_id:class_id}, function(response) {
                $(location).attr('href','<?php echo $link_current; ?>');
