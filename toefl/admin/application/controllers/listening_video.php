...
class Listening_video extends CI_Controller {
    ...
    public function __construct() {
        ...
        //--- load object model
        $this->load->model('Listening_video_model');
    ...
    public function search() {
        ...
        $this->data['items'] = $this->Listening_video_model->search_entries();
    ...
    public function index($start = 0) {
        ...
        $config['total_rows'] = $this->Listening_video_model->count_all_entries();
        ...
        $this->data['items'] = $this->Listening_video_model->get_entries($start);
    ...
    public function add() {
        if (isset($_POST['title'])) {
            ...
            if ($message == '') {
                ...
                $this->Listening_video_model->insert_entry();
    ...
    public function edit($id = 0) {
        ...
        $this->data['id'] = $id;
        $item = $this->Listening_video_model->get_entry($id);
        if (isset($item[0]))
        ...
        if (isset($_POST['title'])) {
            ...
            if ($message == '') {
                $this->Listening_video_model->update_entry();
    ...
    public function confirm_delete($id = 0) {
        ...
        $this->Listening_video_model->delete_entry($id);

        //--- redirect to list page
        redirect($this->data['link_object']);
    }

    public function load_video_with_current_duration() {
        $lid = $this->input->post('lid');
        $current_duration = $this->input->post('current_duration');

        $query = $this->db->query('SELECT * FROM ' . $this->db->dbprefix('listening_video') . ' where lid=' . $lid . ' and time<=' . $current_duration . ' order by time desc limit 0,1');
        foreach ($query->result() as $row) {
            $response['id'] = $row->id;
            $response['title'] = $row->title;
            $response['lid'] = $row->lid;
