...
class Reading extends CI_Controller {
    ...
    private function get_reading($rid) {
        $query = $this->db->query('SELECT * FROM ' . $this->db->dbprefix('kto_toefl_reading') . ' where id=' . $rid);
    ...
    private function get_scq($rid) {
        ...
        $query = $this->db->query('SELECT * FROM ' . $this->db->dbprefix('kto_toefl_reading_scq') . ' where deleted=0 and rid=' . $rid);
    ...
    private function get_mcq($rid) {
        $question;
        $query = $this->db->query('SELECT * FROM ' . $this->db->dbprefix('kto_toefl_reading_mcq') . ' where deleted=0 and rid=' . $rid);
        foreach ($query->result_array() as $rows) {
            $this->number_question++;
            $question[] = array(
                'number_question' => $this->number_question,
    ...
    private function get_iq($rid) {
        $question;
        $query = $this->db->query('SELECT * FROM ' . $this->db->dbprefix('kto_toefl_reading_iq') . ' where deleted=0 and rid=' . $rid);
        foreach ($query->result_array() as $rows) {
            $this->number_question++;
    ...
    private function get_ddq($rid) {
        $question;
        $query = $this->db->query('SELECT * FROM ' . $this->db->dbprefix('kto_toefl_reading_ddq') . ' where deleted=0 and rid=' . $rid);
        foreach ($query->result_array() as $rows) {
            //load choices
            $query1 = $this->db->query('SELECT * FROM ' . $this->db->dbprefix('kto_toefl_reading_ddq_answer') . ' where deleted=0 and ddqid=' . $rows['id']);
            $i = -1;
            
            if(isset($choice)) unset($choice);
            ...
            $query1 = $this->db->query('SELECT * FROM ' . $this->db->dbprefix('kto_toefl_reading_ddq_subjects') . ' where deleted=0 and ddqid=' . $rows['id']);
            $i = -1;
            foreach ($query1->result() as $row1) {
                $i++;
