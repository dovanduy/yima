...
class HelperApp {
    ...
    public static function email($to, $subject, $message, $footer = true, $from = 'no-reply@yima.vn') {
        if ($footer)
            $message .= '';
        //$subject =  $subject;
    ...
    public static function _parse_array_values($array, $key_name, $value_name) {
        $tmp = array();
        foreach ($array as $v)
            $tmp[$v[$key_name]] = $v[$value_name];
        return $tmp;
    }
